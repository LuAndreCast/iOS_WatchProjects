...
PhoneCommunicatorDelegate
{
    ...
    {
        let messageToSend:[String:Any] = [ "Command": "Info"]
        phoneCom.sendMessage(messageToSend: messageToSend, replyHander:
            { (reply:[String : Any]) in
                self.updateMessage(message: "Info Command Reply: \(reply)")
            }, errorHandler: { (error:Error) in
                self.updateMessage(message: "Info Command Error, \(error.localizedDescription)")
        })
    }//eom
